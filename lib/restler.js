...
var requestMethods = {
  ...
  _errorHandler: function(err) {
    ...
    self._respond('error', '', err.message);
  },
  ...
  _createMultipartRequest: function() {
	if(typeof this.headers['Content-Type'] == "undefined") // inject content-type only when not set
  ...
  run: function() {
    ...
    if (this.options.multipart) {
      multipart.write(this.request, this.options.data, multipart.defaultBoundary, function() {
        self.request.end();
      });
    } else {
		this.request.end();
...
var _exports = {
  ...
  file: multipart.file,
  data: multipart.data,
  defaultBoundary: multipart.defaultBoundary
}
